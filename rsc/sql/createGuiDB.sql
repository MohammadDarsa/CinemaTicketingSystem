...
USE `mydb`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mydb`.`ticket_AFTER_INSERT` AFTER INSERT ON `ticket` FOR EACH ROW
BEGIN
...
USE `mydb`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mydb`.`ticket_AFTER_INSERT_1` AFTER INSERT ON `ticket` FOR EACH ROW
BEGIN
...
USE `mydb`$$
DROP TRIGGER IF EXISTS `mydb`.`ticket_AFTER_INSERT_2` $$
USE `mydb`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mydb`.`ticket_AFTER_INSERT_2` AFTER INSERT ON `ticket` FOR EACH ROW
BEGIN
    update seat set seat.taken = "true" where new.seat_id = seat.id;
END$$
